...
reader.on(index_1.RfidReaderEvent.DidSeeTag, (tag) => {
  ...
  // writeToJSON(tag);
  // try {
  //   fs.writeFileSync("tags.json", JSON.stringify(tag));
  // } catch (error) {
  //   console.error(error);
  // }
  try {
    const existedData = fs.readFileSync("./tags.json");

    let existedDataJson = JSON.parse(existedData);

    existedDataJson.push(tag);

    fs.writeFileSync("./tags.json", JSON.stringify(existedDataJson));

    // console.log("EXISTED DATA", JSON.stringify(existedData));
  } catch (error) {
    console.error(error);
  }
  // fs.writeFileSync("./tags/tags.json", JSON.stringify(tag));
